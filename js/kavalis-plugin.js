(function ($) {
    ...
    var tipsOverlayHTML, kvButton, kvTermsOverlay, medicalTermsArray, inspirationalQuotesArray,
        termsModalStart, termsModalEnd, medicalTermsModalContent, medicalTermsHeader;
    ...
    function kv_start() {
        ...
        var kv_toggleTipsOverlay = function () {
            if ($('#instructions-wrapper').css('visibility') === 'visible') {
                $('#instructions-wrapper').css('visibility', 'hidden');
            } else {
        ...
        function kv_addShowTipsOverlayLink() {
            $('.fpd-product-designer-wrapper').before('<div>'
                ...
                + '</div>');
            $('.tips-overlay-link').on('click', kv_toggleTipsOverlay);
        }

        function kv_addTermsOverlayAndButton() {

            var currentPageTermsArray = 'what', path, pathArray, dialogType;
        ...
        function getModalDialogTitle(type) {
            if (type === 'medical') {
